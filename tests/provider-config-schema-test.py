...
check(set(files_config["properties"])=={"version","includeGitIgnored","searchRoots"},"Files configuration contains no machine state")
...
files_roots=pc.validate_config("omalaunch.files",{"version":1,"searchRoots":["/mnt/delta/","~","~/Code/../Work"]},Path("/home/test"))
check(files_roots["searchRoots"]==["/mnt/delta","/home/test","/home/test/Work"],"Files search roots are normalized absolute paths")
validate_files_config=fastjsonschema.compile(files_config)
validate_files_config({"version":1,"includeGitIgnored":False,"searchRoots":["/mnt/delta","~/Code","~"]})
check(True,"Files schema accepts additional search roots")
for bad_roots in (["relative/path"],["/mnt/a","/mnt/a/"],"/mnt/a",[1],["/mnt/x"]*33):
    try: pc.validate_config("omalaunch.files",{"version":1,"searchRoots":bad_roots},Path("/home/test"))
    except ValueError: pass
    else: raise AssertionError("invalid Files search roots passed: "+repr(bad_roots)[:60])
print("ok - Files rejects relative, duplicate, non-list, non-string, and excessive search roots")
